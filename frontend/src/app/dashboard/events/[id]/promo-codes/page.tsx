...
import { getEventById, getPromoCodes } from '@/lib/api';
import { PromoCodesList } from '@/components/dashboard/promo-codes/promo-codes-list';
import { dashboardEventRoute } from '@/lib/routes';

const EventPromoCodesPage = async ({
  params,
}: {
  ...
  const [{ data: event }, { data: codes }] = await Promise.all([
    ...
    getPromoCodes({ path: { eventId: id }, headers }),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-5xl px-4 py-6 md:px-8 md:py-10">
      ...
      <PromoCodesList eventId={id} initialCodes={codes ?? []} />
    </div>
  );
};
