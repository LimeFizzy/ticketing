...
interface EditorCanvasProps {
  ...
  onResizeDecoration: (id: string, x: number, y: number, width: number, height: number) => void;
}
...
export function EditorCanvas({
  ...
  onMovePlace, onMoveDecoration,
  onResizeSection, onResizeDecoration,
}: EditorCanvasProps) {
  ...
  function startDrag(
    ...
    id: string, type: 'place' | 'decoration',
    handle: DragHandle,
    startX: number, startY: number, startW: number, startH: number,
  ) {
    e.stopPropagation();

    if (e.shiftKey && handle === 'move') { onSelect(id, true); return; }
    ...
    for (const sid of dragIds) {
      ...
      if (p) { startPositions[sid] = { x: p.x, y: p.y, w: p.width ?? 0, h: p.height ?? 0, type: 'place' }; }
      else {
        const d = decorations.find((x) => x.id === sid);
        if (d) startPositions[sid] = { x: d.x, y: d.y, w: d.width, h: d.height, type: 'decoration' };
      }
    }
    if (!startPositions[id]) startPositions[id] = { x: startX, y: startY, w: startW, h: startH, type };

    draggingRef.current = {
      primaryId: id, handle,
      startSvgX: svgCoords.x, startSvgY: svgCoords.y,
      startPositions, moved: false,
    };
    forceUpdate((n) => n + 1);
  }
  ...
  const handleSvgPointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (d.handle === 'move') {
      for (const [sid, start] of Object.entries(d.startPositions)) {
        const nx = useSnap ? snapVal(start.x + rawDx) : Math.round(start.x + rawDx);
        const ny = useSnap ? snapVal(start.y + rawDy) : Math.round(start.y + rawDy);
    ...
    if (d.handle === 'se') { nw = Math.max(MIN_SIZE, w + sdx); nh = Math.max(MIN_SIZE, h + sdy); }
    else if (d.handle === 'sw') { const dw = Math.min(w - MIN_SIZE, sdx); nx = x + dw; nw = w - dw; nh = Math.max(MIN_SIZE, h + sdy); }
    else if (d.handle === 'ne') { nw = Math.max(MIN_SIZE, w + sdx); const dh = Math.min(h - MIN_SIZE, sdy); ny = y + dh; nh = h - dh; }
    else if (d.handle === 'nw') { const dw = Math.min(w - MIN_SIZE, sdx); nx = x + dw; nw = w - dw; const dh = Math.min(h - MIN_SIZE, sdy); ny = y + dh; nh = h - dh; }

    if (primary.type === 'decoration') onResizeDecoration(d.primaryId, nx, ny, nw, nh);
    ...
    if (m) {
      ...
      for (const p of places) {
        if (p.kind === 'seat') {
          ...
          const pw = p.width ?? 200, ph = p.height ?? 100;
          if (rectsOverlap(p.x, p.y, pw, ph, rx, ry, rw, rh)) ids.add(p.id);
        }
      }
      for (const d of decorations) {
        if (rectsOverlap(d.x, d.y, d.width, d.height, rx, ry, rw, rh)) ids.add(d.id);
      }
  ...
  const singleSelected = selectedIds.size === 1 ? selectedIds.values().next().value! : null;

  function renderHandles(x: number, y: number, w: number, h: number, id: string, type: 'place' | 'decoration') {
    const corners: { handle: DragHandle; cx: number; cy: number; cursor: string }[] = [
  ...
  const mqRect = mq ? {
    x: Math.min(mq.sx, mq.ex), y: Math.min(mq.sy, mq.ey),
    w: Math.abs(mq.ex - mq.sx), h: Math.abs(mq.ey - mq.sy),
  } : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <defs>
          <pattern id="editor-grid" x="0" y="0" width={SNAP} height={SNAP} patternUnits="userSpaceOnUse">
            <circle cx={SNAP / 2} cy={SNAP / 2} r={0.6} fill="currentColor" className="text-border/60" />
          </pattern>
        </defs>
        <rect x={0} y={0} width={mapWidth} height={mapHeight} className="fill-background" />
        <rect x={0} y={0} width={mapWidth} height={mapHeight} fill="url(#editor-grid)" />
        ...
        {decorations.map((d) => {
          ...
          return (
            <g key={d.id} style={{ cursor: 'grab' }}>
              <rect
                x={d.x} y={d.y} width={d.width} height={d.height} rx={8}
                className={isSelected ? 'fill-muted-foreground/30' : 'fill-muted-foreground/20'}
                stroke={isSelected ? '#2563eb' : '#94a3b8'}
                strokeWidth={isSelected ? 2 : 1.5}
                onPointerDown={(e) => startDrag(e, d.id, 'decoration', 'move', d.x, d.y, d.width, d.height)}
              />
              <text
                x={d.x + d.width / 2} y={d.y + d.height / 2 + 5}
              ...
              {isSelected && singleSelected === d.id && renderHandles(d.x, d.y, d.width, d.height, d.id, 'decoration')}
            </g>
          );
        })}

        {/* Seat places */}
        {places.filter((p) => p.kind === 'seat').map((p) => {
          const isSelected = selectedIds.has(p.id);
          return (
            <g key={p.id} style={{ cursor: 'grab' }}
              onPointerDown={(e) => startDrag(e, p.id, 'place', 'move', p.x, p.y, 0, 0)}
            >
        ...
        {/* Section places */}
        {places.filter((p) => p.kind === 'section').map((p) => {
          const w = p.width ?? 200, h = p.height ?? 100;
          const isSelected = selectedIds.has(p.id);
          const cx = p.x + w / 2, cy = p.y + h / 2;
          return (
            <g key={p.id} style={{ cursor: 'grab' }}>
              <rect
                x={p.x} y={p.y} width={w} height={h} rx={10}
                fill={isSelected ? 'rgba(37,99,235,0.22)' : 'rgba(100,116,139,0.12)'}
                stroke={isSelected ? '#2563eb' : '#64748b'}
                strokeWidth={2}
                strokeDasharray={isSelected ? undefined : '6 3'}
                onPointerDown={(e) => startDrag(e, p.id, 'place', 'move', p.x, p.y, w, h)}
              />
              <text x={cx} y={cy - 6} textAnchor="middle" fontSize={13} fontWeight={600}
                className="fill-foreground" style={{ pointerEvents: 'none', userSelect: 'none' }}>
                {p.label}
              </text>
              <text x={cx} y={cy + 12} textAnchor="middle" fontSize={10}
                className="fill-muted-foreground" style={{ pointerEvents: 'none', userSelect: 'none' }}>
                Cap: {p.capacity}
              </text>
              {isSelected && singleSelected === p.id && renderHandles(p.x, p.y, w, h, p.id, 'place')}
            </g>
          );
        })}
        ...
        {mqRect && mqRect.w > 1 && mqRect.h > 1 && (
          <rect
            x={mqRect.x} y={mqRect.y} width={mqRect.w} height={mqRect.h}
      ...
      {/* Mode toggle */}
      <div className="absolute left-3 top-3 flex gap-1" onPointerDown={(e) => e.stopPropagation()}>
        <Button
